...
namespace PauGO
{
    public partial class Form3 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            // OgrenciNo boşsa işlem yapılmasın
            ...
            try
            {
                ...
                kullaniciKomut.Parameters.AddWithValue("@kTuru", "Ogrenci");
                ...
                SqlCommand ogrenciKomut = new SqlCommand(
                    "INSERT INTO Ogrenci (User_id, OgrenciNo) VALUES (@kID, @ogrNo)", connection, transaction);
                ogrenciKomut.Parameters.AddWithValue("@kID", kullaniciiD);
                ogrenciKomut.Parameters.AddWithValue("@ogrNo", ogrenciNo);

                int affectedRows = ogrenciKomut.ExecuteNonQuery(); // Öğrenci tablosuna veri ekle
                if (affectedRows == 0)
                {
                    transaction.Rollback(); // Eğer ekleme yapılmazsa işlemi geri al
                    MessageBox.Show("Öğrenci kaydını oluştururken bir hata oluştu.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
